...
class VoxelRCNNKLLabelIoUHead(VoxelRCNNHead):
    ...
    def init_reg_std_layer_weights(self):
        ...
        nn.init.constant_(self.reg_std_layer.bias, 0.1) # Init with slightly positive uncertainty (0.1)
    ...
    def get_box_reg_layer_loss(self, forward_ret_dict):
        ...
        if loss_cfgs.REG_LOSS == 'smooth-l1':
            ...
            # Aleatoric Uncertainty Loss (Unsupervised)
            # Loss = exp(-s) * L_reg + 0.5 * s
            ...
            rcnn_loss_reg_log = (0.5 * rcnn_reg_std \
                 * fg_mask.unsqueeze(dim=-1).float()).sum() / max(fg_sum, 1) * loss_cfgs.LOSS_WEIGHTS['rcnn_reg_weight']
